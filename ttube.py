...
HELP = "Enter: search/play  ↑↓: move  Esc: search  Tab: switch  P/Space: pause  S: stop  Q: quit"
HELP2 = "Seek: ←/→ 5s   [ / ] 10s   PgUp/PgDn 30s   Home/End"
SPINNER = "|/-\\"
...
def _draw(stdscr, app: App) -> None:
    ...
    try:
        stdscr.hline(2, min(len(HELP2) + 1, max(0, w - 1)), curses.ACS_HLINE, max(0, w - len(HELP2) - 2))
    except Exception:
        pass

    # Search bar
    query_label = "Search" + ("*" if app.mode == "query" else "")
    _safe_addstr(stdscr, 4, 0, f"{query_label}: ")
    ...
    if app.now_playing:
        ...
        _safe_addstr(stdscr, 5, 0, f"Now playing: {app.now_playing}{paused}"[: max(0, w - 1)], np_attr)
        ...
        _safe_addstr(stdscr, 6, 0, line, curses.A_DIM)
        ...
        _safe_addstr(stdscr, 7, 0, vu_line, curses.A_DIM)

    # Results
    results_y = 8 if app.now_playing else 7
    results_header = f"Results ({len(app.results)})" + ("*" if app.mode == "results" else "")
    _safe_addstr(stdscr, results_y, 0, results_header[: max(0, w - 1)], curses.A_BOLD)

    list_top = results_y + 1
    max_rows = max(0, h - list_top - 2)
    ...
    if app.results and max_rows > 0:
        ...
        if len(app.results) > max_rows:
            pos = f"{app.selected + 1}/{len(app.results)}"
            _safe_addstr(stdscr, results_y, max(0, w - len(pos) - 1), pos, curses.A_DIM)

    # Status bar
    status_attr = 0
    if curses.has_colors():
        if app.status.lower().startswith("search failed") or app.status.lower().startswith("playback failed"):
...
def _handle_input(ch: int, app: App) -> bool:
    ...
    # Pause: P always works; Space only when focus is results
    if ch in (ord("p"), ord("P")) or (ch == ord(" ") and app.mode == "results"):
    ...
    if app.mode == "query":
        ...
        # Accept printable chars INCLUDING space
        if 32 <= ch <= 126:
            app.query += chr(ch)
            return True
